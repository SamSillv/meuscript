...
class IQOptionAPI(object):  # pylint: disable=too-many-instance-attributes
    ...
    def send_http_request(self, resource, method, data=None, params=None, headers=None):  # pylint: disable=too-many-arguments
        ...
        response = self.session.request(method=method,
                                        ...
                                        timeout=getattr(self, "http_timeout", 20))
    ...
    def send_websocket_request(self, name, msg, request_id="", no_force_send=True):
        ...
        # Não aguardar indefinidamente enquanto o callback recebe mensagens.
        # Após o limite, esta única chamada é enviada; o backend não reenvia
        # a ordem se a confirmação não chegar, evitando duplicidade.
        limite_espera = time.time() + 5
        while (global_value.ssl_Mutual_exclusion or global_value.ssl_Mutual_exclusion_write) and no_force_send:
            if time.time() >= limite_espera:
                logger.warning("Timeout aguardando callback WebSocket; enviando a solicitação uma única vez.")
                break
            time.sleep(0.001)
        global_value.ssl_Mutual_exclusion_write = True
        socket_obj = self.websocket
        timeout_anterior = None
        try:
            if hasattr(socket_obj, "gettimeout"):
                timeout_anterior = socket_obj.gettimeout()
            if hasattr(socket_obj, "settimeout"):
                socket_obj.settimeout(10)
            socket_obj.send(data)
            logger.debug(data)
        finally:
            if timeout_anterior is not None and hasattr(socket_obj, "settimeout"):
                try:
                    socket_obj.settimeout(timeout_anterior)
                except Exception:
                    pass
...
# -------------------------------------------------------
    ...
    def start_websocket(self, timeout=30):
        ...
        self.websocket_thread = threading.Thread(target=self.websocket.run_forever, kwargs={'sslopt': {
                                                 "check_hostname": False, "cert_reqs": ssl.CERT_NONE, "ca_certs": "cacert.pem"}})  # for fix pyinstall error: cafile, capath and cadata cannot be omitted
        ...
        while time.monotonic() < deadline:
            try:
                ...
                pass
            time.sleep(0.01)
        return False, "Timeout aguardando conexão WebSocket."
    ...
    def send_ssid(self, timeout=20):
        self.profile.msg = None
        self.ssid(global_value.SSID)  # pylint: disable=not-callable
        deadline = time.monotonic() + max(1.0, float(timeout))
        while self.profile.msg is None and time.monotonic() < deadline:
            time.sleep(0.01)
        if self.profile.msg is None:
            logging.warning("Timeout aguardando confirmação do SSID.")
            return False
    ...
    def connect(self):
        ...
        while self.timesync.server_timestamp is None and time.monotonic() < deadline:
            time.sleep(0.01)
        if self.timesync.server_timestamp is None:
            return False, "Timeout aguardando sincronização de horário do WebSocket."
        return True, None
    ...
    def close(self):
        try:
            ...
            pass
        thread = getattr(self, "websocket_thread", None)
        if thread is not None and thread.is_alive():
            thread.join(timeout=5)
            if thread.is_alive():
                logging.warning("WebSocket anterior não encerrou no prazo; seguindo sem bloquear a nova sessão.")
